...
public class sorts {
  ...
  public static int[] countingSort(int[] keys, int whichDigit) {
	  ...
	  int n = keys.length;
	  int[] eachDigit = new int[n];
	  
	  //>>(int)(Math.pow(4,whichDigit)
	  //the faster way is to use bit operation
	  int shift = 4*whichDigit;
	  System.out.println("shift:"+shift);
	  for(int i = 0; i<n; i++){
		  eachDigit[i] = ((keys[i]>>shift))&15;
		  
		  //eachDigit[i] = (int) (keys[i]%(Math.pow(10, whichDigit+1))/(Math.pow(10, whichDigit)));
		  //System.out.println("each:"+eachDigit[i]);
	  }
	  //counting the key in keys
