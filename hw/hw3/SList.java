...
public class SList {
  ...
  public void squish() {
	...
	while(test.next!=null){
		if(test.item.equals(test.next.item)){
  ...
  public void twin() {
	...
	if(head!=null){
		while(test.next!=null){
			tag=test.next;
			SListNode doubleL=new SListNode(test.item);    //***** the difference between reference and object
			doubleL.next=test.next;
			test.next=doubleL;
			test=tag;
		}
		//the last node
		SListNode doubleL=new SListNode(test.item);    //*****
		doubleL.next=null;
		test.next=doubleL;	
	}
