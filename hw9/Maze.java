...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    DisjointSets set = new DisjointSets(horiz*vert);  // the size is the same as each array(hWalls or vWalls)
    int numHwalls = horiz*(vert-1);
    int numVwalls = vert*(horiz-1);
    //order the interior walls of the maze in a random order.
    int[] array = new int[(horiz-1)*vert+(vert-1)*(horiz)];
    
    //map the hWalls to first part of array 
    for(i=0; i<numHwalls; i++){
    	array[i] = i+1;
    }
    //map the vWalls to second part of array AND distinguish them with the +-
    for(i=numHwalls;i<numHwalls+numVwalls;i++){
    	array[i] = numHwalls-i-1;
    }
    //algorithmic (i begin from here because of the radInt)
    for(int w=numHwalls+numVwalls; w>0; w--){
    	int index = randInt(w); //from 0 to w-1
    	//System.out.println(w+ " " + index);
    	int temp = array[index]; //w walls
    	array[index] = array[w-1]; //w-1 walls
    	array[w-1] = temp;
    }
    
    //visit the real walls in the (random) order where they appear
    //in the array
    for(i = 0; i<numHwalls+numVwalls; i++){
    	j = array[i];
    	if(j > 0){ //hwalls (0,0) (1,0) (2,0) -+ -+  note: reverse the x,y
    		int topX = (j-1)/(vert -1);
    		int topY = (j-1)%(vert -1);
    		int downX = topX;
    		int downY = topY+1;
    		int topCell = topX + topY * horiz;
    		int downCell = downX + downY * horiz; //remap done
    		//System.out.println(topX+" "+topY);
    		System.out.println(""+topCell+" "+downCell);
    		if(set.find(topCell) != set.find(downCell)){//not in the same set
    			System.out.println(" "+topCell+" "+downCell);
    			hWalls[topX][topY] = false;  //real path  cancel
    			set.union(set.find(topCell), set.find(downCell)); //tag
    		}
    	} else{  //vWalls
    		int leftX = (-j-1) / (horiz - 1);
            int leftY = (-j-1) % (horiz - 1);
            int rightX = leftX + 1;
            int rightY = leftY;
            int leftCell = leftX + leftY * horiz;
            int rightCell = rightX + rightY * horiz;
            System.out.println("  "+leftCell+" "+rightCell);
