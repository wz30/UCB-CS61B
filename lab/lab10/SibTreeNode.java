...
class SibTreeNode extends TreeNode {
  ...
  public TreeNode parent() throws InvalidNodeException {
	...
	if(isValidNode()){
		if(this.equals(myTree.root)) //this.equals(mytree.root);
			return new SibTreeNode();
		else return parent;   //depressed parent();
	}
	else
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
	...
	SibTreeNode insertedNode = new SibTreeNode(myTree,item);
	if(!isValidNode()){  //not valid Node -- no item
		throw new InvalidNodeException();
	}
	else{
	if(c<=1){                  //firstchild
		insertedNode.nextSibling = firstChild;
		insertedNode.parent= this;
		firstChild = insertedNode;
		myTree.size++;
	}
	else if(c>this.children()){  //add the last position//find the last//
		SibTreeNode countNode = firstChild;
	    while (countNode.nextSibling != null) {
	        countNode = countNode.nextSibling;
	      }           
	    countNode.nextSibling = insertedNode;
	    insertedNode.parent = countNode.parent;
	    myTree.size++;
	}
	else{                   //insert
		SibTreeNode countNode2 = (SibTreeNode)child(c-1); //use ?? not SibTreeNode
		insertedNode.parent = countNode2.parent;
		insertedNode.nextSibling = countNode2.nextSibling;
		countNode2.nextSibling = insertedNode;
	    myTree.size++;
	}
	}
  }
  ...
  public void removeLeaf() throws InvalidNodeException {
    // FILL IN YOUR SOLUTION TO PART III HERE.
	  if(!isValidNode()){  //not valid Node -- no item
		  ...
		  if(this.children()==0&&myTree.size!=1){  //if leaf
			  if(this==this.parent.firstChild){   //firstChild
				  ...
				  SibTreeNode brotherNode = this.parent.firstChild;
				  while(brotherNode.nextSibling!=this){
					  brotherNode=brotherNode.nextSibling;
				  }
				  brotherNode.nextSibling = this.nextSibling;
				  this.parent = null;
				  this.valid = false;
				  myTree.size--;
			  }
		  }
		  else if(myTree.size==1){  //only root
			  myTree.root = null;
			  myTree.size--;
		  }
	  }
