...
public class UDGraph
{
  ...
  public UDGraph length2Paths() {
    ...
    //operate the this ;not newGraph
    int row = this.adjMatrix.length;
    int column = this.adjMatrix[0].length;
    for(int i = 0; i<row ;i++){
    	for(int j = 0; j<column; j++){
    		  if(this.adjMatrix[i][j]==true){
    			for(int k = 0; k<row ;k++){
    				if(this.adjMatrix[j][k]==true){  //ab bc(a)
  ...
  public UDGraph paths(int length) {
    ...
    if(length == 1) return this;//return length2Paths();
    
    //the graph length of n-1
    UDGraph newGraph2 = paths(length-1);
    
    int row = this.adjMatrix.length;
    int column = this.adjMatrix[0].length;
    for(int i = 0; i<row ;i++){
    	for(int j = 0; j<column; j++){
    		  if(newGraph2.adjMatrix[i][j]==true){
    			for(int k = 0; k<row ;k++){
    				if(this.adjMatrix[j][k]==true){  //ab bc(a)
    					if(newGraph.adjMatrix[i][k]!=true)
    					{
    					newGraph.adjMatrix[i][k]=true;
    					newGraph.edges++;
    					}
